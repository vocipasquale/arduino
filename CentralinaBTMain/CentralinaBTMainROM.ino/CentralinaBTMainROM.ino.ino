 ...
 */
    ...
    if(input.startsWith(SET_STATUS_1)){
      setStatusOutuput(1, input.substring(SET_STATUS_1.length()+2), true);
    }

    if(input.startsWith(SET_STATUS_2)){
      setStatusOutuput(2, input.substring(SET_STATUS_2.length()+2), true);
    }

    if(input.startsWith(SET_STATUS_3)){
      setStatusOutuput(3, input.substring(SET_STATUS_3.length()+2), true);
    }

    if(input.startsWith(SET_STATUS_4)){
      setStatusOutuput(4, input.substring(SET_STATUS_4.length()+2), true);
    }
 ...
 */
void setStatusOutuput(int o, String statusOutput, boolean saveInEPROM){
  Alarm alarms[NUM_ALARM];
...
//legge dalla EPROM i valori salvati e li riporta sulle uscite
void loadFromEEPROM(){
  int outputSize = 59; // size di getStatusOutput esclusi i primi 4 char, ved. persistIntoEEPROM()
  int totalSize = outputSize * NUM_OUTPUT;
  byte buff[outputSize];
  int index = 0;
  int o = 1;

  //Serial.print("totalSize: ");
  //Serial.println(totalSize);
  Serial.println("leggo dalla EPROM...");
  for(int i=1; i<=totalSize; i++){
    if(index < outputSize){
     // Serial.print("index: ");
     // Serial.println(index);
      buff[index] = EEPROM.read(i);
      index++;  
    }else{//index == 59;
   ...
   //   Serial.println(String((char *)buff));
      setStatusOutuput(o, String((char *)buff), false);
      o++;
      index = 0;
      buff[index] = EEPROM.read(i);
      index++;
...
//salva nella EPROM i valori degli allarmi ed il nome di ogni uscita
void persistIntoEEPROM(int o){
  String output = getStatusOutput(o);
  int startIndexEPROM = 1; //l'index 0 è riservato all'informazione di EPROM valorizzata o no
  int outputSize = 59;
  char chars[output.length()-4];
  
  for(int i=4; i<output.length(); i++){
    chars[i-4]=output.charAt(i);
  }
    
//  Serial.print("salvo in EPROM: ");
//  Serial.println(chars);

    for(int i=0; i<sizeof(chars); i++){
      EEPROM.update(i+startIndexEPROM+((o-1)*outputSize), (byte)chars[i]);
    }
  
  //imposto 1 per indicare che la EPROM contiene dei dati salvati
  EEPROM.update(0, 1);
}
